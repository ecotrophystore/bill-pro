 ...
 */
async function ensureUserRegistered(uid: string, email: string) {
  try {
...
async function getRevenueData() {
    ...
    invSnap.forEach(d => {
        const data = d.data();
        if (data.status !== "cancelled") { invRevenue += data.subtotal || 0; invGst += data.tax_total || 0; invCount++; }
    });
    memoSnap.forEach(d => {
        const data = d.data();
        memoRevenue += data.subtotal || 0; memoGst += data.tax_total || 0; memoCount++;
    });
...
async function getPendingInvoices() {
    ...
    return { count: results.length, total_pending: results.reduce((s: number, r: any) => s + (r.amount || 0), 0), invoices: results.slice(0, 10) };
}
...
async function getProfitData() {
    ...
    invSnap.forEach(d => { if (d.data().status !== "cancelled") revenue += d.data().subtotal || 0; });
...
async function countDocuments(args: { collection_name: string }) {
    const validCollections = ["invoices", "cash_memos", "quotations", "products", "customers", "purchases"];
    const col = args.collection_name;
    if (!validCollections.includes(col)) return { error: `Invalid collection. Valid: ${validCollections.join(", ")}` };
    const snap = await db.collection(col).get();
    return { collection: col, count: snap.size };
}

async function searchInvoicesByCustomer(args: { customer_name: string }) {
...
async function getBusinessSummary() {
    ...
    invSnap.forEach(d => {
        const data = d.data();
        if (data.status !== "cancelled") { revenue += data.subtotal || 0; gst += data.tax_total || 0; }
        if (data.payment_status === "unpaid" || data.payment_status === "partial") unpaid += data.grand_total || 0;
    });
...
export const voiceCommander = onCall({ 
    ...
    const voiceModel = ai.getGenerativeModel({
        ...
        systemInstruction: `# ════════════════════════════════════════════════════════════════
# ECOBILL PATTU — MASTER SYSTEM & VOICE PROMPT
# Platform  : Antigravity · Model: Gemini Flash
# ════════════════════════════════════════════════════════════════

## IDENTITY & SOUL
You are **Pattu**, the AI Co-Founder of Bill Pro. You are a business partner, not a chatbot.
Tone: Confident, warm, direct. Like a trusted partner who has been working with the owner for years.
Tamil Context: Call the owner "Anna" or "Akka" when appropriate. Use VERY colloquial/native Tamil slang.

## VOICE CORE RULES (CRITICAL)
1. Responses must be SHORT (max 2-3 lines). No markdown, no bullet points, no bolding. Plain text only.
2. User is SPEAKING. If it sounds weird when read aloud, rewrite it.
3. Detect language first: Tamil, Tanglish, or English. Lock your response to that language.
4. One question at a time. If data is missing for a form, ask for the most important field first (e.g., Customer Name).
5. Understand messy input: Ignore fillers (um, ah). Handle self-corrections (e.g., "5... no 8 bags").
6. Phonetic Tamil: Recognize "invoice podu" as "போடு", "vilaipattiyil" as "விலைப்பட்டியல்", etc.

## BUSINESS INTELLIGENCE & FORM FILLING
1. Form Fields: "customerName", "productName", "quantity", "price", "hsn", "gst".
2. Navigation: Use 'navigate' tool for pages: dashboard, invoices, quotations, cash-memos, products, customers, purchases, reports, auditor.
3. Magic Fill: Call 'update_form_field' immediately for extracted data.
4. New Entities: Use 'open_overlay' (customer/product) for new entries.
5. Confirmation: Once filled, call 'request_confirmation' and ask "Confirm pannalaamaa?" or "Shall I confirm?".
6. Data Queries: Use tools like 'get_revenue', 'get_profit', 'get_pending_invoices' for business stats.

## AUDITOR MODE
Triggered by "Auditor" or "Risk check". Shift to a more formal CA persona. Flag high debt, overdue invoices (>60 days), and GST deadlines.

## EXAMPLE (TANGLISH)
User: "Pattu, Raju-ku invoice create pannu"
Pattu: "Kandippa Anna! Raju-oda invoice-la enna item add panna-num?"`
    }, { apiVersion: 'v1beta' });
    ...
    if (calls && calls.length > 0) {
        ...
        for (const call of calls) {
            ...
            if (call.name === "navigate") {
                ...
                toolResult = { success: true, navigated_to: (call.args as any).page };
            }
            ...
            else if (call.name === "get_revenue") toolResult = await getRevenueData();
            else if (call.name === "get_pending_invoices") toolResult = await getPendingInvoices();
            else if (call.name === "get_profit") toolResult = await getProfitData();
            else if (call.name === "count_documents") toolResult = await countDocuments(call.args as any);
            else if (call.name === "search_invoices_by_customer") toolResult = await searchInvoicesByCustomer(call.args as any);
            else if (call.name === "get_business_summary") toolResult = await getBusinessSummary();
