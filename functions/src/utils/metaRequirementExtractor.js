 ...
 */
export function extractMetaRequirements(text) {
    ...
    if (occasionMatch && occasionMatch[1])
        noteParts.push(`Occasion: ${occasionMatch[1].trim()}`);
    const recogniseMatch = raw.match(/^[•\*\-\s]*(?:Who\s*we’re\s*recognising|Recognising)\s*[:=-]\s*(.+)$/im);
    if (recogniseMatch && recogniseMatch[1])
        noteParts.push(`Recognising: ${recogniseMatch[1].trim()}`);
    const awardCategoriesMatch = raw.match(/^[•\*\-\s]*Award\s*categories\s*[:=-]\s*(.+)$/im);
    if (awardCategoriesMatch && awardCategoriesMatch[1])
        noteParts.push(`Awards: ${awardCategoriesMatch[1].trim()}`);
    const deadlineFlexMatch = raw.match(/^[•\*\-\s]*Deadline\s*flexibility\s*[:=-]\s*(.+)$/im);
    if (deadlineFlexMatch && deadlineFlexMatch[1])
        noteParts.push(`Deadline: ${deadlineFlexMatch[1].trim()}`);
    const logoMatch = raw.match(/^[•\*\-\s]*(?:References\s*or\s*logo|Logo)\s*[:=-]\s*(.+)$/im);
    if (logoMatch && logoMatch[1])
        noteParts.push(`Logo: ${logoMatch[1].trim()}`);
...
//# sourceMappingURL=metaRequirementExtractor.js.map