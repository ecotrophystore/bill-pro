...
export async function processWebhookPayload(eventId, data) {
    ...
    try {
        ...
        if (eventType === "whatsapp_message" && value.messages && value.messages.length > 0) {
            ...
            if (requirements.required_quantity)
                leadSnapshot.required_quantity = requirements.required_quantity;
            if (requirements.urgency)
                leadSnapshot.urgency = requirements.urgency;
            if (requirements.customer_type)
                leadSnapshot.customer_type = requirements.customer_type;
            if (requirements.location)
                leadSnapshot.location = requirements.location;
            ...
            if (!isRepeat) {
                ...
                if (requirements.required_quantity)
                    newLeadData.required_quantity = requirements.required_quantity;
                if (requirements.event_name)
                    newLeadData.event_name = requirements.event_name;
                if (requirements.event_type)
                    newLeadData.event_type = requirements.event_type;
                if (requirements.event_date)
                    newLeadData.event_date = requirements.event_date;
                if (requirements.delivery_date)
                    newLeadData.delivery_date = requirements.delivery_date;
                if (requirements.trophy_type)
                    newLeadData.trophy_type = requirements.trophy_type;
                if (requirements.trophy_size)
                    newLeadData.trophy_size = requirements.trophy_size;
                if (requirements.budget)
                    newLeadData.budget = requirements.budget;
                if (requirements.value)
                    newLeadData.value = requirements.value;
                if (requirements.email)
                    newLeadData.email = requirements.email;
                if (requirements.notes)
                    newLeadData.reason = requirements.notes;
                if (requirements.organization)
                    newLeadData.company = requirements.organization;
                if (requirements.customer_type)
                    newLeadData.customer_type = requirements.customer_type;
                if (requirements.urgency)
                    newLeadData.urgency = requirements.urgency;
                if (requirements.location)
                    newLeadData.location = requirements.location;
                ...
                if (leadDoc) {
                    ...
                    if (requirements.customer_name && (!existingData.name || existingData.name.startsWith("Lead +") || existingData.name.toLowerCase().includes("event")))
                        updates.name = requirements.customer_name;
                    if (requirements.organization && !existingData.company)
                        updates.company = requirements.organization;
                    if (requirements.email && !existingData.email)
                        updates.email = requirements.email;
                    if (requirements.budget && !existingData.budget)
                        updates.budget = requirements.budget;
                    if (requirements.value && !existingData.value)
                        updates.value = requirements.value;
                    if (requirements.location && !existingData.location)
                        updates.location = requirements.location;
                    if (requirements.required_quantity && !existingData.required_quantity)
                        updates.required_quantity = requirements.required_quantity;
                    if (requirements.event_name && !existingData.event_name)
                        updates.event_name = requirements.event_name;
                    if (requirements.event_type && !existingData.event_type)
                        updates.event_type = requirements.event_type;
                    if (requirements.delivery_date && !existingData.delivery_date)
                        updates.delivery_date = requirements.delivery_date;
                    if (requirements.urgency)
                        updates.urgency = requirements.urgency;
        ...
        if (eventType === "leadgen") {
            ...
            if (!token)
                throw new Error("META_FACEBOOK_SYSTEM_USER_TOKEN not configured.");
...
//# sourceMappingURL=metaWebhookProcessor.js.map