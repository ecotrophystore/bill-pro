...
export const getMetaSecretStatus = onCall(
  { secrets: [metaAppSecret, metaVerifyToken, waToken, fbToken] },
  async (request) => {
    ...
    try { if (waToken.value()) whatsappTokenConfigured = true; } catch (e) {}
    try { if (fbToken.value()) facebookTokenConfigured = true; } catch (e) {}
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (entries.length > 0 || body.object) {
          ...
          const batch = db.batch();
          const eventsToProcess: { id: string, data: any }[] = [];
...
// ── diagnoseMetaAccess ───────────────────────────────────────────────────────
export const diagnoseMetaAccess = onCall(
  { secrets: [fbToken, waToken] },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Must log in");
    await requireAdmin(request.auth.uid);

    const config = await db.collection("meta_integrations").doc("default").get().then(d => d.data() || {});
    const report: any = { status: "success", checks: [] };
    ...
    try {
      ...
      try {
        const debugInfo = await graphGet<any>(`/debug_token`, fb, { input_token: fb });
    ...
    try {
      ...
      try {
        const debugInfo = await graphGet<any>(`/debug_token`, wa, { input_token: wa });
        const isValid = debugInfo.data?.is_valid;
        const scopes = debugInfo.data?.scopes || [];
        report.checks.push({ 
...
// ── syncMetaTemplates ────────────────────────────────────────────────────────
export const syncMetaTemplates = onCall(
  { secrets: [waToken] },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Must log in");
    await requireAdmin(request.auth.uid);
    ...
    try {
      const token = await resolveWhatsAppAuthorization();
      const response = await graphGet<any>(`/${version}/${config.whatsappBusinessAccountId}/message_templates`, token);
...
export const syncBusinessProfileToMeta = onCall(
  { secrets: [waToken] },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Must log in");
    await requireAdmin(request.auth.uid);
    ...
    if (!profile) throw new HttpsError("invalid-argument", "Profile data missing");
    ...
    try {
      ...
      if (profile.category) payload.vertical = profile.category;
      if (profile.email) payload.email = profile.email;
      if (profile.website) payload.websites = [profile.website];
      
      await graphPost<any>(
        `/${version}/${config.whatsappPhoneNumberId}/whatsapp_business_profile`,
        token,
        payload
      );
