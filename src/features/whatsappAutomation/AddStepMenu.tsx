...
export function AddStepMenu({ open, anchorLabel, allowedTypes, onChoose, onClose }: Props) { if (!open) return null; return <div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-[2px]" onClick={onClose}><div className="absolute z-50 w-[320px] rounded-3xl border border-shadow-darker/10 bg-transparent shadow-2xl p-3" style={{ left: '50%', top: '30%', transform: 'translate(-50%, -50%)' }} onClick={(event) => event.stopPropagation()}><div className="px-2 pb-3 border-b border-shadow-darker/10"><p className="text-xs uppercase tracking-[0.2em] text-secondary">Add step after</p><p className="font-semibold text-primary-dark">{anchorLabel}</p></div><div className="mt-3 space-y-2">{allowedTypes.map((type) => <button key={type} type="button" onClick={() => onChoose(type)} className="w-full rounded-2xl border border-shadow-darker/10 bg-transparent px-4 py-3 text-left text-sm font-medium text-primary-dark hover:border-primary/20 hover:bg-primary/5 transition-all">{labels[type]}</button>)}</div></div></div>;
}
