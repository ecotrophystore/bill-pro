...
export default function CreateInvoice() {
  ...
  useEffect(() => {
    if (lastAction && window.location.pathname.includes('/invoices/new')) {
  ...
  const handleGenerateInvoice = async () => {
    if ((!selectedCustomerId && !selectedCustomerName) || !auth?.currentUser || !functions) {
      showToast('Please select or enter a customer and ensure you are logged in.', 'error');
      return;
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-primary-dark">Create Invoice</h1>
          <p className="text-secondary mt-1">Directly generate a Tax Invoice. Locked upon creation.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className={`neo-card p-4 space-y-3 shadow-[5px_5px_15px_#D1C4B2,-5px_-5px_15px_#FFFFFF] ${confirmationRequested ? 'ring-4 ring-primary animate-pulse' : ''}`}>
            <button 
              disabled={isSaving}
              onClick={() => {
              ...
              className="neo-btn-primary w-full py-4 flex items-center justify-center gap-3 text-lg font-bold shadow-lg"
            >
              {isSaving ? <Loader2 size={24} className="animate-spin" /> : <>Generate Tax Invoice <Sparkles size={20}/></>}
            </button>
          </div>
        </div>
      </div>
