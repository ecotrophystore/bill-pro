...
export default function Login() {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      setError("Demo credentials are not set up. Please use your own email/password or configure demo accounts in Firebase Auth.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface w-full p-4">
      <div className="w-full max-w-md neo-card animate-fade-in flex flex-col gap-6">
        ...
        <button 
          ...
          disabled={loading}
          className="w-full py-2 px-4 rounded-xl border-2 border-primary/20 text-primary font-bold hover:bg-primary/5 transition-colors duration-200 disabled:opacity-50"
        >
          Quick Demo Login
        </button>
